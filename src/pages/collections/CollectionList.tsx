...
export default function CollectionList() {
  ...
  const fetchCollections = async () => {
    ...
    try {
      ...
      const data = await ApiService.getCollectionsWithPagination(
        page,
        pageSize,
        debouncedSearchTerm,
      ...
      const mapped = data.results.map((c: any) => ({
        ...
        creation_date: c.creation_date || c.created_at || c.createdAt || "",
        updation_date: c.updation_date || c.updated_at || c.updatedAt || "",
  ...
  const getImagePreview = (collection: Collection) => {
    ...
    if (
      ...
      console.log("Filepath", collection.image);
    ...
    if (typeof collection.image === "string") {
      ...
      console.log("editedCollection.image", collection.image);
  ...
  const handleDeleteCollection = async (collection: Collection) => {
    ...
    try {
      ...
      toast({
        ...
        description: `"${collection.name}" has been deleted successfully.`,
      });
    } catch (err) {
      toast({
        title: "Delete Failed",
        description: `Failed to delete collection: ${collection.name}`,
        variant: "destructive",
      });
    } finally {
      setDeletingId(null);
    ...
    <Card className="group hover:shadow-md transition-shadow duration-200">
      <div className="relative">
        <div className="aspect-video bg-muted rounded-t-lg flex items-center justify-center overflow-hidden">
          {collection.image ? (
            <img
              src={getImagePreview(collection)}
              alt={collection.name}
      ...
      <CardContent className="pt-0">
        ...
        <div className="flex items-center justify-between">
          <Badge variant={getStatusVariant(collection.is_active)}>
  ...
  const handleExportCollections = async () => {
    ...
    return await ApiService.exportCollections(
    ...
    <div className="space-y-6 p-6">
      <PageHeader
        ...
        description={`Manage your product collections (${
          debouncedSearchTerm
            ? `${collectionsToDisplay.length} of ${totalCount}`
            : totalCount
        } collections)`}
        actions={
          canManageCollections() && (
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <ExportButton
                ...
                filename={`collections-export-${new Date().toISOString().split("T")[0]}`}
                variant="outline"
              >
                Export Collections
              </ExportButton>
