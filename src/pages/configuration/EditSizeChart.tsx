...
const EditSizeChart = () => {
  ...
  const { data: sizeChartData, isLoading: sizeChartLoading } = useQuery({
    queryKey: ["size-chart", id],
    queryFn: () => ApiService.getSizeChart(Number(id)),
    enabled: !!id,
  });
  ...
  // Update form data when size chart data is loaded
  useEffect(() => {
    if (sizeChartData) {
      setFormData({
        category: sizeChartData.category.id.toString(),
        attribute: sizeChartData.attribute.id.toString(),
        name: sizeChartData.name,
        description: sizeChartData.description,
        is_active: sizeChartData.is_active,
      });

      // Set selected category for hierarchical dropdown
      setSelectedCategory({
        id: sizeChartData.category.id.toString(),
        name: sizeChartData.category.name,
        level: 2, // This will be updated by the dropdown component
      });
  ...
  const updateSizeChartMutation = useMutation({
    mutationFn: (data: SizeChartRequest) =>
      ApiService.updateSizeChart(Number(id), data),
    onSuccess: () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateSizeChartMutation.mutate(sizeChartData);
  };
    ...
    return (
      <div className="space-y-6 p-6">
        <PageHeader
          ...
          description="Update size chart with measurements for a specific category and attribute"
        />
        <Card>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Update basic details for the size chart
            </CardDescription>
          </CardHeader>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Update the measurements for this size chart
                </CardDescription>
              </div>
              <Button
                type="button"
        ...
        <div className="flex items-center justify-end space-x-4">
          ...
          <Button type="submit" disabled={updateSizeChartMutation.isPending}>
